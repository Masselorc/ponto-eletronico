...
# Alias para a rota listar_feriados para compatibilidade
@admin.route('/admin/feriados')
@login_required
def feriados():
...
def novo_feriado():
    ...
    if form.validate_on_submit():
        # CORREÇÃO: Verificar se já existe um feriado com a mesma data
        feriado_existente = Feriado.query.filter_by(data=form.data.data).first()
        if feriado_existente:
            flash(f'Já existe um feriado cadastrado para a data {form.data.data.strftime("%d/%m/%Y")}: {feriado_existente.descricao}', 'danger')
...
def editar_feriado(feriado_id):
    ...
    if form.validate_on_submit():
        # CORREÇÃO: Verificar se já existe outro feriado com a mesma data
        feriado_existente = Feriado.query.filter_by(data=form.data.data).first()
        if feriado_existente and feriado_existente.id != feriado.id:
            flash(f'Já existe um feriado cadastrado para a data {form.data.data.strftime("%d/%m/%Y")}: {feriado_existente.descricao}', 'danger')
