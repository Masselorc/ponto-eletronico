...
def dashboard():
    ...
    try:
        ...
        for dia_num in range(1, ultimo_dia.day + 1):
            ...
            # Verifica se é dia da semana (0-4) E não é feriado
            if data_atual.weekday() < 5 and data_atual not in feriados_datas:
                dias_uteis_potenciais += 1 # Conta como dia útil potencial
                registro_dia = registros_dict.get(data_atual)
                # Verifica se existe registro E se é um afastamento
                if registro_dia and registro_dia.afastamento:
                    dias_afastamento += 1 # Conta como dia de afastamento

        # 2. Calcular dias trabalhados e horas trabalhadas (apenas em dias não afastados)
        # Percorre os registros *existentes* para somar horas e dias trabalhados
        for r_data, r_obj in registros_dict.items(): # Usa o dict para evitar reprocessar
            # Verifica se NÃO é afastamento E se tem horas registradas
            if not r_obj.afastamento and r_obj.horas_trabalhadas is not None:
                # Verifica se o dia do registro era realmente um dia útil potencial (redundante mas seguro)
        ...
        # Carga devida = (Dias úteis potenciais - Dias de afastamento nesses dias) * 8 horas
        ...
        return render_template(
            'main/dashboard.html',
            registros=registros, # Mantém a lista original para a tabela
            mes_atual=mes_req, ano_atual=ano_req, nome_mes=nome_mes,
            dias_uteis=dias_uteis_potenciais, # Envia dias úteis *potenciais* como 'dias_uteis'
...
def relatorio_mensal():
    ...
    try:
        ...
        # Busca ordenada por data para a tabela do relatório
        registros = Ponto.query.filter(
            Ponto.user_id == usuario_ctx.id,
            Ponto.data >= primeiro_dia,
            Ponto.data <= ultimo_dia
        ...
        registros_por_data = {registro.data: registro for registro in registros} # Dict para cálculo e template
        ...
        for r_data, r_obj in registros_por_data.items(): # Usa dict para cálculo
            if not r_obj.afastamento and r_obj.horas_trabalhadas is not None:
                 if r_data.weekday() < 5 and r_data not in feriados_datas:
                    dias_trabalhados += 1
                    horas_trabalhadas += r_obj.horas_trabalhadas
        ...
        return render_template(
            ...
            registros=registros, # Lista ordenada para a tabela
            mes_atual=mes_req, ano_atual=ano_req, nome_mes=nome_mes,
            mes_anterior=mes_anterior, ano_anterior=ano_anterior,
            proximo_mes=proximo_mes, proximo_ano=proximo_ano,
            dias_uteis=dias_uteis_potenciais, # Envia dias úteis *potenciais*
            ...
            registros_por_data=registros_por_data, # Dict usado no loop do template
...
def registrar_multiplo_ponto():
    ...
    if form.validate_on_submit():
        try:
            ...
            if registros_criados > 0 and registros_ignorados == 0: flash(f'{registros_criados} registro(s) criado(s) com sucesso!', 'success')
            elif registros_criados > 0 and registros_ignorados > 0: flash(f'{registros_criados} registro(s) criado(s). {registros_ignorados} data(s) ignorada(s) por já existir registro.', 'warning')
            elif registros_ignorados > 0 and registros_criados == 0: flash('Nenhum novo registro criado (todas as datas informadas já possuíam registro).', 'info')
            elif len(datas_processadas) == 0: flash('Nenhuma data válida foi informada para registro.', 'warning')
            else: flash('Nenhum registro criado devido a erros nos dados informados.', 'danger')
