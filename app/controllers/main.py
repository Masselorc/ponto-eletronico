...
# --- CORREÇÃO: Importar MultiploPontoForm ---
...
# ... (Funções auxiliares e outras rotas mantidas como antes) ...
...
def dashboard():
    ...
    except Exception as e: logger.error(f"Erro dashboard: {e}", exc_info=True); flash('Erro ao carregar dashboard.', 'danger'); return render_template('main/dashboard.html', registros=[], mes_atual=hoje.month, ano_atual=hoje.year, nome_mes="Mês Atual", dias_uteis=0, dias_trabalhados=0, dias_afastamento=0, horas_trabalhadas=0, carga_horaria_devida=0, saldo_horas=0, media_diaria=0, usuario=current_user, usuarios=None)
...
def registrar_ponto():
    ...
    if form.validate_on_submit():
        try:
            data_selecionada = form.data.data; registro_existente = Ponto.query.filter_by(user_id=current_user.id, data=data_selecionada).first()
            ...
            novo_registro = Ponto(user_id=current_user.id, data=data_selecionada, entrada=form.entrada.data, saida_almoco=form.saida_almoco.data, retorno_almoco=form.retorno_almoco.data, saida=form.saida.data, horas_trabalhadas=horas_calculadas, observacoes=form.observacoes.data, afastamento=False, tipo_afastamento=None)
            ...
            db.session.commit(); flash('Registro criado!', 'success'); return redirect(url_for('main.dashboard', mes=data_selecionada.month, ano=data_selecionada.year))
...
def editar_ponto(ponto_id):
    ...
    if request.method == 'GET' and atividade_existente and not form.atividades.data: form.atividades.data = atividade_existente.descricao
    if form.validate_on_submit():
        try:
            ...
            if is_afastamento and not tipo_afastamento: flash('Selecione Tipo Afastamento.', 'danger'); return render_template('main/editar_ponto.html', form=form, registro=registro, title="Editar Registro")
            registro.data = data_selecionada; registro.afastamento = is_afastamento; registro.tipo_afastamento = tipo_afastamento; registro.observacoes = form.observacoes.data
...
def registrar_multiplo_ponto():
    ...
    # --- CORREÇÃO: Instanciar o formulário minimalista ---
    form = MultiploPontoForm()
    # ----------------------------------------------------

    # --- CORREÇÃO: Usar form.validate_on_submit() para validar CSRF ---
    if form.validate_on_submit(): # Substitui request.method == 'POST'
    # -------------------------------------------------------------
        try:
            ...
            for i in range(len(datas_str)):
                ...
                except ValueError: flash(f'Data inválida ignorada: {data_str}', 'warning'); continue
                ...
                if registro_existente: flash(f'Registro para {data.strftime("%d/%m/%Y")} já existe.', 'info'); registros_ignorados += 1; continue
                ...
                except ValueError: flash(f'Hora inválida para {data.strftime("%d/%m/%Y")}.', 'warning'); continue
            ...
            # Redireciona de volta para o formulário, mas o form não será repopulado automaticamente aqui
            return redirect(url_for('main.registrar_multiplo_ponto'))

    # Se GET ou se a validação CSRF falhar (form.validate_on_submit() retorna False)
    # --- CORREÇÃO: Passar o form para o template ---
...
def registrar_atividade(ponto_id): # Assinatura corrigida
    ...
    if form.validate_on_submit():
        try:
            ...
            else: db.session.add(Atividade(ponto_id=ponto_id, descricao=descricao))
