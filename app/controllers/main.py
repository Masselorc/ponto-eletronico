...
def editar_ponto(ponto_id):
    ...
    if form.validate_on_submit():
        ...
        registro.atividades = form.atividades.data
...
def calendario():
    ...
    # Calcula o mês anterior e o mês seguinte para navegação
    if mes_atual == 1:
        mes_anterior = 12
    ...
    # CORREÇÃO: Passar as classes timedelta e date para o template
    return render_template('main/calendario.html',
                          ...
                          timedelta=timedelta,
                          date=date)  # CORREÇÃO: Passar a classe date
    ...
    if request.method == 'POST':
        ...
        for i in range(len(datas)):
            ...
            try:
                ...
                if i < len(saidas_almoco) and saidas_almoco[i]:
                    saida_almoco = datetime.strptime(saidas_almoco[i], '%H:%M').time()
                if i < len(retornos_almoco) and retornos_almoco[i]:
